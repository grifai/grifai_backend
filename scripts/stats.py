...
Использование (из корня проекта):
  python scripts/stats.py            — всё
  python scripts/stats.py contacts   — только контакты
  python scripts/stats.py stats      — только статистика по действиям
  python scripts/stats.py recent     — последние 20 взаимодействий
  python scripts/stats.py search Маша — найти контакт
  python scripts/stats.py profile Маша — профиль контакта
"""
...
if __name__ == "__main__":
    if not MEMORY_FILE.exists():
        print(f"{MEMORY_FILE} не найден. Запустите python -m app.main сначала.")
        sys.exit(1)
